 ...
 * @author Bruce
 */
public class PipeRistic extends Ristic.RisticClass
{
	
	static public Ristic.RisticClass newRisticClass(Context ctxt, Symbol name, ParserPoint point)
		{ return (new PipeRistic(name, point)); }
	
	protected PipeRistic(Symbol name, ParserPoint point)
		{ super(name, point); }
	...
	{ 
		...
		if (elements.hasVariableElements() || elements.isEmpty())
			return (Ristic.newRistic(this, elements, parms.parserPoint()));
		PipeRistics.checkPipeOptions(ctxt, false, elements, parms);
		return (new Pipe(ctxt, this, elements, parms));
	}
	
	static public class Pipe extends PipeRistics
	{
		public Pipe(Context ctxt, RisticClass rClass, Elements elements, List source)
			throws YagaException
		...
		public Pipeline newPipeline(List[] es, Variable.PipeVariable[] sinks, boolean flHaveRoot, ParserPoint point)
			{ return (new Pipeline(this, Elements.make(es, this), sinks, flHaveRoot, point)); }
	}
}
